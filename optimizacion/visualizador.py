...
class Visualizador:
    ...
    def hacer_linea(self, mapa, coordenadas_i, coordenadas_j, i, j, color_linea='blue'):
        folium.PolyLine(locations=[(coordenadas_i[i-1][1], coordenadas_i[i-1][2]),
                                    (coordenadas_j[j-1][1], coordenadas_j[j-1][2])],
                        color=color_linea,
                        weight=4).add_to(mapa)
        
    def escoger_color(self, i):
        color_linea = 'blue'
        tipo_vehiculo = self.p.vehiculos["VehicleType"][i]
                ...
                for v in self.M.V:
                    if i != j and self.M.Z[i,j,v]() == 1:
                        self.hacer_linea(mapa, coordenadas_clientes, coordenadas_clientes, i, j, self.escoger_color(v))
                         # no entiendo pq este no sirve
                        self.hacer_linea(mapa, coordenadas_clientes, coordenadas_clientes, 1, 2, self.escoger_color(v))
